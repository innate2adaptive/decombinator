...
# NOTES:
# modified from Demultiplexor.py for new oligo design (produced at beginning of sequence read 2):
# original oligo -> I8.6N.I8.6N
# new oligo -> M13.6N.I8.6N
# lines modified are commented as such:
 # modified for oligo using M13 sequence
# added SP1 index '8' <- 'ACTTGA'
...
# Takes all three reads and simultaneously demultiplexes and formats read 1 for vDCR.py analysis
  # i.e. adds the first 42 nucleotides (modified for oligo using M13 sequence) of R2 to R1
...
### NB For non-standard Illumina encoded fastqs, might need to change which fields are carried into fq_* vars
  ...
  Nseq = record3[1][0:42] # modified for oligo using M13 sequence
  X1seq = record1[1][6:12]
  X2seq = record2[1]
  readseq = record1[1][12:]

  Nqual = record3[2][0:42] # modified for oligo using M13 sequence
