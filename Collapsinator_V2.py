...
def get_barcode(bcseq):
        ...
        if "N" in bcseq and inputargs['allowNs'] == False:    # ambiguous base-call check 
          ...
          return
        ###########################################################################################################
        #area between ### lines has been removed and replaced by script which only looks for one barcode.
        ...
        # otherwise look throughout the entire sequence for the presence of two spacers
          ...
          # bcseq = ("X"*pad) + bcseq 
        #############################################################################################################
          #extract bases for second spacer only
            bcseq_ss = bcseq[12:26]

        #search for exact match in this region 
            err_spcrs = regex.findall(spcr, bcseq_ss) 
